...
point_on_plate = (0.055, 0.025, 0.0)
...
edge_on_bottomface1 = holeInstance.edges.findAt(((0.015, 0.0, 0.0),))
edge_on_bottomface2 = holeInstance.edges.findAt(((0.021, 0.0, 0.0),))
edge_on_bottomface3 = holeInstance.edges.findAt(((0.04, 0.0, 0.0),))
...
edge_on_leftface1 = holeInstance.edges.findAt(((0.0, 0.015, 0.0),))
edge_on_leftface2 = holeInstance.edges.findAt(((0.0, 0.021, 0.0),))
edge_on_leftface3 = holeInstance.edges.findAt(((0.0, 0.04, 0.0),))
...
# vertex_for_force = (12.0, 0.0, 0.0)
# force_vertex = holeInstance.vertices.findAt((vertex_for_force,))
# holeModel.ConcentratedForce(name='Load', createStepName='Load Step', region=(force_vertex,),
#                                 cf1=160.0, distributionType=UNIFORM)

Loading_edge1 = holeInstance.edges.findAt(((0.12, 0.04, 0.0),))
Loading_edge2 = holeInstance.edges.findAt(((0.12, 0.015, 0.0),))

loadingface_region1 = regionToolset.Region(side1Edges=Loading_edge1)
loadingface_region2 = regionToolset.Region(side1Edges=Loading_edge2)

holeModel.Pressure(name='Load-1', createStepName='Load Step', region=loadingface_region1, distributionType=UNIFORM,
                   field='', magnitude=-1.6e8, amplitude=UNSET)
holeModel.Pressure(name='Load-2', createStepName='Load Step', region=loadingface_region2, distributionType=UNIFORM,
                   field='', magnitude=-1.6e8, amplitude=UNSET)
...
plate_region = (face_in_square, face_in_square2, face_in_tri, face_in_other1, face_in_other2, face_in_other3)
...
pickedRegionsQuad = holePart.faces.findAt((point_in_square,), (point_in_square2,), (point_in_other1,),
                                          (point_in_other2,), (point_in_other3,))
...
holePart.seedEdgeByNumber(edges=edge_in_circle2, number=40, constraint=FINER)

holePart.seedEdgeByBias(biasMethod=SINGLE, end2Edges=edge_in_square1, ratio=5.0, number=40, constraint=FINER)
holePart.seedEdgeByBias(biasMethod=SINGLE, end2Edges=edge_in_square2, ratio=5.0, number=40, constraint=FINER)
holePart.seedEdgeByBias(biasMethod=SINGLE, end1Edges=edge_in_square3, ratio=5.0, number=40, constraint=FINER)
